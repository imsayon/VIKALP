// Flow C — CUDA execution backend implementation.
//
// Architecture rules:
...
//   - cudaMalloc/cudaFree never happen inside inner solver loops.
...
public:
    explicit CudaVector(Index n)
        : n_(n), buf_(static_cast<std::size_t>(n)) {
        if (n < 0) throw std::invalid_argument("CudaVector: negative size");
...
public:
    explicit CudaMatrix(const CsrPattern &pat, cusparseHandle_t handle)
        : rows_(pat.rows), cols_(pat.columns),
          nnz_(static_cast<Index>(pat.column_indices.size())),
          d_row_offsets_(static_cast<std::size_t>(pat.rows + 1)),
          d_col_indices_(pat.column_indices.size()),
          d_values_(pat.column_indices.size()),
          handle_(handle) {
        ...
        // Zero-initialise values
        check_cuda(cudaMemset(d_values_.ptr, 0, nnz_ * sizeof(double)),
                   "CudaMatrix zero values");

        // Create cuSPARSE sparse matrix descriptor (CSR, int64 indices, FP64)
        check_cusparse(
            cusparseCreateCsr(
                &desc_,
                rows_, cols_, nnz_,
...
private:
    ...
    cusparseHandle_t handle_; // not owned
...
public:
    CudaBackend() {
        ...
        // Allocate small persistent reduction workspace
        check_cuda(cudaMalloc(&d_reduce_buf_, kReduceBufSize * sizeof(double)),
                   "alloc reduce buf");
        check_cuda(cudaMalloc(&h_scalar_pinned_, sizeof(double)),
                   "alloc pinned scalar");
    }

    ~CudaBackend() {
        cudaFree(d_reduce_buf_);
        cudaFree(h_scalar_pinned_);
    ...
    create_matrix(const CsrPattern &pattern) override {
        return std::make_unique<CudaMatrix>(pattern, sparse_handle_);
    }
    ...
    [[nodiscard]] Scalar dot(const BackendVector &x,
        ...
        // blocks result stored in d_reduce_buf_; second pass on host
    ...
    void multiply(Scalar alpha, const BackendMatrix &matrix,
        ...
        cusparseDnVecDescr_t dx_desc, dy_desc;
        check_cusparse(cusparseCreateDnVec(&dx_desc, xv.size(),
                                           const_cast<double *>(xv.device_ptr()),
                                           CUDA_R_64F),
                       "CreateDnVec x");
        check_cusparse(cusparseCreateDnVec(&dy_desc, yv.size(),
        ...
        check_cusparse(cusparseSpMV_bufferSize(
                           ...
                           dx_desc, &beta, dy_desc, CUDA_R_64F,
        ...
        std::unique_ptr<double, decltype(&cudaFree)> tmp_workspace(nullptr, cudaFree);
        if (buf_size > spmv_workspace_size_) {
            double *p = nullptr;
            check_cuda(cudaMalloc(&p, buf_size), "SpMV workspace alloc");
            tmp_workspace.reset(p);
            workspace = p;
            // Cache for next call if this grows
            if (d_spmv_workspace_) cudaFree(d_spmv_workspace_);
            d_spmv_workspace_ = p;
            tmp_workspace.release(); // now owned by d_spmv_workspace_
        ...
        cusparseDestroyDnVec(dx_desc);
        cusparseDestroyDnVec(dy_desc);
    }
    ...
    solve_linear_system(const BackendMatrix &matrix,
        ...
        res.converged = false;
        res.iterations = iteration_limit;
        return res;
    }
...
private:
    ...
    // Persistent reduction buffer — large enough for most problem sizes
    static constexpr std::size_t kReduceBufSize = 4096;
    double *d_reduce_buf_ = nullptr;
    double *h_scalar_pinned_ = nullptr; // pinned host memory for async scalars
