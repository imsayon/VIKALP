...
SolveResult solve_scaled_lp(const Model &model, ExecutionBackend &backend,
    ...
    // Row/column equilibration above supplies diagonal preconditioning.
    const Scalar matrix_norm = detail::scaled_operator_bound(scaled.pattern, scaled.values);
    const Scalar max_step = 0.99 / std::max<Scalar>(1.0, matrix_norm);
    Scalar tau = max_step;
    ...
    // Restarted PDHG: all numerical work stays behind ExecutionBackend.
    for (Index iter = 0; iter < options.iteration_limit; ++iter) {
        ...
        backend.axpby(-tau, *objective, 1.0, *x_next);
        backend.axpby(-tau, *aty, 1.0, *x_next);
        ...
        backend.multiply(1.0, *matrix, *y, 0.0, *aty, Transpose::Yes);
        backend.copy(*objective, *gradient);
    ...
    if (!std::isfinite(result.objective_value)) {
        ...
        result.message = "PDHG produced a non-finite objective";
    }
    ...
    std::vector<Scalar> stationarity = model.linear_objective;
    for (Index row = 0; row < m; ++row) {
        ...
        for (Index k = model.constraint_matrix.pattern.row_offsets[static_cast<std::size_t>(row)];
            ...
            stationarity[static_cast<std::size_t>(model.constraint_matrix.pattern.column_indices[position])] +=
                multiplier * model.constraint_matrix.values[position];
        }
    }
    // Bounds use the projected-gradient KKT residual, rather than a raw gradient.
    ...
    for (Index i = 0; i < n; ++i) {
        ...
        const auto projected = std::clamp(
            solution[position] - stationarity[position], bounds.lower[position],
        ...
        if (std::isfinite(bounds.lower[position]) &&
            ...
            result.variable_lower_dual[position] = std::max<Scalar>(0.0, stationarity[position]);
        ...
        if (std::isfinite(bounds.upper[position]) &&
            ...
            result.variable_upper_dual[position] = std::max<Scalar>(0.0, -stationarity[position]);
            result.complementarity_residual = std::max(
                result.complementarity_residual,
    ...
    if (!detail::finite_solution(result.primal_solution)) {
        result.status = SolveStatus::NumericalFailure;
        result.message = "PDHG produced a non-finite iterate";
        ...
        result.status = SolveStatus::NumericalFailure;
        result.message = "PDHG stopped without satisfying original-coordinate residual checks";
    }
...
SolveResult solve_lp(const Model &model, ExecutionBackend &backend,
                     const SolverOptions &options,
                     std::span<const Scalar> warm_start,
    ...
    auto validation = detail::validate_continuous(model, ProblemClass::LP, bounds);
    if (validation.status != SolveStatus::NotSolved) return validation;
    if (model.num_variables() == 0) {
        return detail::invalid_result("LP requires at least one variable");
    }
    ...
    if (options.continuous_method != ContinuousMethod::Auto &&
        ...
        return detail::unsupported_result("requested continuous method is not implemented for LP");
    }
    return solve_scaled_lp(model, backend, options, warm_start, bounds);
}

SolveResult LpRelaxationOracle::solve(
    ...
    return solve_lp(model, *backend, options, warm_start,
