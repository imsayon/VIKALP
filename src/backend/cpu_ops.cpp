// Flow B — CPU execution backend implementation.
//
// Design principles:
...
//   - solve_linear_system uses BiCGSTAB.
...
std::size_t checked_vector_size(Index size) {
    if (size < 0) dim_error("vector size must be non-negative");
...
void validate_pattern(const CsrPattern &pattern) {
    ...
    if (pattern.row_offsets.front() != 0 ||
        ...
        dim_error("CsrPattern row_offsets do not describe column_indices");
    ...
    for (Index row = 0; row < pattern.rows; ++row) {
        ...
        if (begin < 0 || begin > end ||
            ...
            dim_error("CsrPattern row_offsets must be ordered and in range");
        Index previous = -1;
        for (Index position = begin; position < end; ++position) {
            ...
            if (column < 0 || column >= pattern.columns || column <= previous)
                dim_error("CsrPattern columns must be ordered and in range");
            previous = column;
        }
    }
}
...
public:
    explicit CpuVector(Index n) : data_(checked_vector_size(n), 0.0) {}
...
// Working vectors are local to one solve and do not leak into solver policy.
// ──────────────────────────────────────────────────────────────────────────────
...
static LinearSolveResult bicgstab(const CpuMatrix &A,
    ...
    Scalar initial_norm = 0.0;
    for (const Scalar value : r)
        initial_norm = std::max(initial_norm, std::abs(value));
    if (initial_norm <= tol)
...
public:
    ...
    //
    // Solver policy decides whether and when to call this numerical operation.
    ...
    solve_linear_system(const BackendMatrix &matrix,
        ...
        if (!std::isfinite(tolerance) || tolerance < 0.0)
            dim_error("solve_linear_system: tolerance must be finite and non-negative");
        if (iteration_limit < 0)
            dim_error("solve_linear_system: iteration_limit must be non-negative");
