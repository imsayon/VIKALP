// Flow B — CPU backend correctness tests.
//
...
void test_create_vector(vikalp::ExecutionBackend &be) {
    ...
    bool threw = false;
    try { (void)be.create_vector(-1); }
    catch (const std::invalid_argument &) { threw = true; }
    check_true("create_vector: reject negative size", threw);
}
...
void test_create_matrix(vikalp::ExecutionBackend &be) {
    ...
    bool threw = false;
    try { (void)be.create_matrix({1, 1, {0, 2}, {0}}); }
    catch (const std::invalid_argument &) { threw = true; }
    check_true("create_matrix: reject malformed CSR", threw);
}
...
void test_solve_linear_system(vikalp::ExecutionBackend &be) {
    ...
    auto zero_rhs = be.create_vector(2);
    auto zero_solution = be.create_vector(2);
    const auto zero_result = be.solve_linear_system(
        *m, *zero_rhs, *zero_solution,
        vikalp::MatrixProperty::General, 1e-10, 100);
    check_true("solve_zero_rhs: converged", zero_result.converged);
    check_true("solve_zero_rhs: zero iterations", zero_result.iterations == 0);

    vikalp::CsrPattern breakdown_pattern{2, 2, {0, 2, 4}, {0, 1, 0, 1}};
    auto breakdown_matrix = be.create_matrix(breakdown_pattern);
    const std::vector<double> breakdown_values = {-3.0, -3.0, -3.0, 0.0};
    be.set_values(std::span<const double>(breakdown_values), *breakdown_matrix);
    auto breakdown_rhs = be.create_vector(2);
    auto breakdown_solution = be.create_vector(2);
    const std::vector<double> breakdown_b = {-3.0, 0.0};
    be.upload(std::span<const double>(breakdown_b), *breakdown_rhs);
    const auto breakdown_result = be.solve_linear_system(
        *breakdown_matrix, *breakdown_rhs, *breakdown_solution,
        vikalp::MatrixProperty::General, 1e-10, 10);
    check_true("solve_breakdown: reports non-convergence",
               !breakdown_result.converged);
    check_true("solve_breakdown: reports attempted iteration",
               breakdown_result.iterations == 1);
    check_eq("solve_breakdown: finite residual", breakdown_result.residual, 3.0);
}
